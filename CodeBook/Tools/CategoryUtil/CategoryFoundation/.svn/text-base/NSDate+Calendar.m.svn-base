...
- (NSUInteger)weekOfDayInYear {
    return [[NSCalendar currentCalendar] ordinalityOfUnit:NSCalendarUnitWeekOfYear inUnit:NSCalendarUnitWeekOfYear forDate:self];
}
...
/******************************************
 *@Description:获取后 三个月的第一天
 *@Params:nil
 *@Return:后三个月的第一天
 ******************************************/
- (NSDate *)firstDayOfTheThirdMonth {
    NSDateComponents *components = [[NSDateComponents alloc] init];
    components.month = 3;
 ...
 *  @return 天数
 */
+ (NSUInteger)numberDaysInCurrentMonth:(NSUInteger)month withYear:(NSUInteger)year
{
